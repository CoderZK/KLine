...
- (void)creatView {
    ...
    if (self.tyep == 1){
        ...
        self.textView.frame = CGRectMake(15, viewMargin, K_Width - 30 - 40, viewHeight);
        self.textView.placeHolderLB.frame = CGRectMake(3, 0, K_Width - 30 - 6 - 40, viewHeight);
    }else {
        self.moreBtn.hidden = YES;
        self.textView.frame = CGRectMake(15, viewMargin, K_Width - 30, viewHeight);
        self.textView.placeHolderLB.frame = CGRectMake(3, 0, K_Width - 30 - 6, viewHeight);
    }
    
    
    //加号按钮
    self.moreBtn.frame = CGRectMake(CGRectGetMaxX(self.textView.frame) + viewMargin, self.height - viewMargin - viewHeight, 40, viewHeight);
...
- (void)setNomalImage:(UIImage *)nomalImage {
    ...
    [self.moreBtn setImage:self.nomalImage forState:UIControlStateNormal];
 
}
- (void)setSelectImage:(UIImage *)selectImage {
    _selectImage = selectImage;
    [self.moreBtn setImage:self.selectImage forState:UIControlStateSelected];
}

- (void)setTyep:(NSInteger)tyep {
    _tyep = tyep;
    [self creatView];
}

- (void)setPlacehodleStr:(NSString *)placehodleStr {
    _placehodleStr = placehodleStr;
    self.textView.placeHolderLB.text = placehodleStr;
}
...
- (void)moreBtn:(UIButton *)btn {
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(didClickRightBt:)]) {
        [self.delegate didClickRightBt:btn];
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if ([text isEqualToString:@"\n"]){
        ...
        textView.text = @"";
        self.textView.placeHolderLB.hidden = NO;
...
- (UIButton *)moreBtn {
    if (!_moreBtn) {
        ...
        [_moreBtn setBackgroundImage:self.nomalImage forState:UIControlStateNormal];
        [_moreBtn setBackgroundImage:self.selectImage forState:UIControlStateNormal];
