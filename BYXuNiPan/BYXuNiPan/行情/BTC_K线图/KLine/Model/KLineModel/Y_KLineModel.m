...
//            第一个循环结束后，ClockFirstValue为最小值
            ...
            NSInteger temp = models.count > 7 ? 7:models.count-1;
            
            for (NSInteger j = temp; j >= 1; j--)
            {
                if (temp <= 1) {
                    return;
                }
...
//                    NSLog(@"\n== emMaxValue ==== %f",emMaxValue.floatValue);
                    em--;
                }
//                NSLog(@"\n emMaxValue %f",emMaxValue.floatValue);
