...
//        _scrollView.alwaysBounceHorizontal = YES;
        ...
        UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(event_tap)];
...
- (Y_KLineMAView *)kLineMAView
{
    if (!_kLineMAView) {
        ...
        _kLineMAView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.85];
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress
{
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state)
    {
        ...
        if(!self.verticalView)
        {
            ...
            CGFloat hh = self.scrollView.height - 30;
...
//                make.bottom.equalTo(@(-30));
        ...
        if (rightXPosition-offsetX < self.scrollView.frame.size.width/2.0) {
            //点击的靠右边
            if (_kLineMAView.frame.origin.x == 0) {
                
                [_kLineMAView mas_remakeConstraints:^(MASConstraintMaker *make) {
                    make.top.equalTo(self);
        ...
        [self sendNotificationWithTitleStr:@"stop"];
...
//长按,拖拽时的发送通知
- (void)sendNotificationWithTitleStr:(NSString *)title {
    
    NSNotification * notification = [NSNotification notificationWithName:title object:nil];
